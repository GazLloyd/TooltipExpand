...
public class GuiTooltipExpand extends GuiScreenWidget {

    private String name;
    private String enabledName;
    private GuiCCButton toggleButton;
    private OptionTooltipExpand opt;
    private Point dragDown;

    public GuiTooltipExpand(OptionTooltipExpand opt, String enabledName) {
        super(80, 20);
        this.opt = opt;
        name = opt.name;
    ...
    public void actionPerformed(String ident, Object... params) {
        if (ident.equals("show")) {
            opt.getTag(enabledName).setBooleanValue(!show());
            updateNames();
        }
    }
    ...
    private boolean show() {
        return opt.getTag(enabledName).getBooleanValue();
    }
    ...
    public void drawScreen(int mousex, int mousey, float f) {
        ...
        if (show())
            HUDRenderer.renderOverlay(new ItemStack(Items.diamond_pickaxe), Arrays.asList("Diamond Pickaxe", EnumChatFormatting.RED + "Sample"), renderPos());
    }
